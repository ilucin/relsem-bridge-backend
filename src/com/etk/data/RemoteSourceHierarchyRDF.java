...
public class RemoteSourceHierarchyRDF extends AbstractRemoteSourceRDF{	
	...
	public List<Object> getEntityCandidates(Properties queryProperties) {
		...
		queryString += "WHERE { ";
  	    ...
  	    if( queryProperties.getEntitySuperClass() != null ){
  	    	queryString += "?" + entityVariable + " rdfs:subClassOf{" + 
  	    				   queryProperties.getDepthFrom() + "," + 
  	    				   queryProperties.getDepthTo() + "} " +
  	    				   "<" + queryProperties.getEntitySuperClass() + ">. }";
  	    }
  	    else{
  	    	queryString += "?s rdfs:subClassOf ?" + entityVariable + ". " +
					   	   "FILTER NOT EXISTS { ?" + entityVariable + " rdfs:subClassOf ?o2 }. }";
  	    }  	   
